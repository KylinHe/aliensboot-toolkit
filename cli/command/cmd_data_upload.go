...
func uploadDir(remoteRoot string, localRoot string) {
	...
	for _, fi := range rd {
		...
		if err != nil {
			log.Fatal("invalid dat file %v", filePath)
			continue
		}
		...
		log.Infof("uploading data %v", remotePath)

		count++

		//上传数据
		result := registry.UploadData(remotePath, data)
		if result {
			success ++
			//log.Infof("upload data %v success", remotePath)
		}
	}
